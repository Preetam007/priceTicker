...
const webhooks = {
	...
	messageHandler : function messageHandler(req,res) {
        ...
        function getMeetUps(data) {
            ...
            req.app.client.get(`${data.lat.toFixed(3)}meetups${data.long.toFixed(3)}`,function getMeetups1(err,reply) {
                ...
                request(options, function getMeetups2(error, response, body) {
                    if (error) {
                        sendMessage({sender : data.sender ,text : 'No Meetups found for now in next 1 weeek'});
                    }
                    else {
                        const pglen = (JSON.parse(body)).results.length;
                        let reducedArray = (JSON.parse(body)).results.slice(lastIndex,lastIndex+limit).reduce(function getMeetups2(arr,curr,i) {
                        ...
                        req.app.client.set(`${data.lat.toFixed(3)}meetups${data.long.toFixed(3)}`,JSON.stringify(messages.attachment),function getMeetups4(err,reply) {
                            //console.log(err);
                            if(err) {
                                console.log(err);
                                console.log('error');
